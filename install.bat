...
:etapa2
ECHO # DESEJA INSTALAR O MAP-OS RELEASE, MASTER OU USAR ARQUIVO LOCAL?
ECHO 1- Release (Versao Estavel)
ECHO 2- Master (Versao Desenvolvimento)
ECHO 3- Usar arquivo local (MapOS.zip)
ECHO 9- Sair
ECHO.
CHOICE /C 1239 /M "Digite uma opcao:"
...
IF ERRORLEVEL 2 (
    SET downMapos=master 
    GOTO etapa0
)
IF ERRORLEVEL 1 (
    SET downMapos=release
    GOTO etapa0
)
GOTO etapa0
...
:: Nova seção para arquivo local
IF "%downMapos%"=="local" (
    :selecionarArquivo
    CLS
    ECHO.
    SET /p "userFilePath=Informe o caminho completo para o arquivo MapOS.zip: "
    IF NOT EXIST "%userFilePath%" (
        ECHO Arquivo nao encontrado! Tente novamente.
        TIMEOUT /T 2 >NUL
        GOTO selecionarArquivo
    )
    COPY /Y "%userFilePath%" "%dirDefault%\MapOS.zip" >NUL
    IF ERRORLEVEL 1 (
        ECHO Falha ao copiar o arquivo. Verifique as permissoes.
        TIMEOUT /T 2 >NUL
        GOTO selecionarArquivo
    )
